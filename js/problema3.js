...
function problema3() {
    ...
    document.querySelector('#p3-output').textContent ="Respuesta: ";
...
function acomodarpalabra(palabra){
    var separador2 = ""
    var arraydeletras = palabra.split(separador2);
    const unicos = [];

    for(var i = 0; i < arraydeletras.length; i++) {
        
        const elemento = arraydeletras[i];
        
        if (!unicos.includes(arraydeletras[i])) {
            unicos.push(elemento);
        }
        }
    var juntaletras =""
    for(var i = 0; i < unicos.length; i++) {   
    juntaletras = juntaletras+unicos[i];
    }
    
    document.querySelector('#p3-output').textContent =document.querySelector('#p3-output').textContent+"\n"+ juntaletras.length+"("+juntaletras+")"+"\n";
}